...
struct NVIC_Type
{
    ...
    void setPriority(IRQn IRQn, uint priority)
    {
        if (IRQn < 0)
        {
            volatileStore(cast(ubyte*)&SCB.SHP[(cast(uint)(IRQn) & 0xF) - 4],
                    cast(byte)((priority << (8 - __NVIC_PRIO_BITS)) & 0xff)); // set Priority for Cortex-M  System Interrupts
        }
        else
        {
            volatileStore(cast(ubyte*)(&NVIC.IP[cast(uint)(IRQn)]),
                    cast(ubyte)((priority << (8 - __NVIC_PRIO_BITS)) & 0xff)); // set Priority for device specific Interrupts
        }
    }

    void enable(byte IRQChannel, byte preemptionPriority, byte subPriority)
    {
        byte tmppriority = 0x00, tmppre = 0x00, tmpsub = 0x0F;

        /* Compute the Corresponding IRQ Priority --------------------------------*/
        tmppriority = (0x700 - ((SCB.AIRCR.load()) & cast(uint) 0x700)) >> 0x08;
        tmppre = cast(byte)(0x4 - tmppriority);
        tmpsub = tmpsub >> tmppriority;

        tmppriority = cast(byte)(preemptionPriority << tmppre);
        tmppriority |= cast(byte)(subPriority & tmpsub);

        tmppriority = cast(byte)(tmppriority << 0x04);

        NVIC.IP[IRQChannel] = tmppriority;

        /* Enable the Selected IRQ Channels --------------------------------------*/
        NVIC.ISER[IRQChannel >> 0x05] = cast(uint) 0x01 << (IRQChannel & cast(byte) 0x1F);

    }

    void disable(byte IRQChannel)
    {
        /* Disable the Selected IRQ Channels -------------------------------------*/
        NVIC.ICER[IRQChannel >> 0x05] = cast(uint) 0x01 << (IRQChannel & cast(byte) 0x1F);
    }
}
